...
@given(datetimes())
def test_get_beta_always_between_m_90_and_90(when_utc):
    nss = non_sun_synchronous()
    assert -90 <= nss.get_beta(when_utc) <= 90
...
def test_get_eclipse_duration_changes_for_non_sun_synchronous_satellite():
    ...
    maximum_eclipse_duration = eclipse_duration(0, nss.period)

    eclipse_duration1 = nss.get_eclipse_duration(dt1)
    eclipse_duration2 = nss.get_eclipse_duration(dt2)
